...
import { Mail, Send, MapPin, Clock, User, MessageSquare, Zap, Phone, Globe,Linkedin } from "lucide-react";
...
const Contact = () => {
  ...
  const quickStats = [
    { label: "Response Time", value: "< 24hrs", icon: Clock, color: "text-purple-400" },
    { label: "Location-Bengaluru", value: "Remote Ready", icon: MapPin, color: "text-purple-500" },
    { label: "Availability", value: "Full-Time", icon: User, color: "text-purple-400" },
    { label: "Time Zone", value: "IST (UTC+5:30)", icon: Globe, color: "text-purple-500" }
  ];
  ...
  const contactMethods = [
    {
      ...
      action: () => window.open('mailto:sanitprakash@outlook.com?subject=Let\'s build something amazing together&body=Hi Sanit,\n\nI\'d like to discuss...', '_blank'),
    ...
    },
    {
      ...
      action: () => window.open('https://www.google.com/maps/place/Basaweshwara+Nagar,+Bengaluru,+Karnataka', '_blank'),
    ...
    },
     {
      ...
      action: () => window.open('https://www.linkedin.com/in/sanit-prakash/', '_blank'),
      gradient: "from-purple-600 to-purple-700"
    },
  ];

  return (
    <section id="contact" className="py-16 md:py-20 px-4 sm:px-6 bg-black min-h-screen relative">
        ...
        <div className="grid lg:grid-cols-3 gap-8 md:gap-12">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="project-card wakanda-pattern">
              ...
              {status && (
                <div className={`mt-4 md:mt-6 p-3 md:p-4 rounded-lg text-center font-medium text-sm md:text-base transition-all ${
                  status.includes('successfully') ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'
                }`}>
                  {status}
                </div>
              )}
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              {contactMethods.map(({ icon: Icon, title, value, action, gradient }) => (
                <button
                  key={title}
                  onClick={action}
                  className={`project-card p-4 md:p-6 text-center hover:scale-105 transition-all duration-300 bg-gradient-to-br ${gradient}/10 border-purple-500/20 hover:border-purple-400/40 group`}
                >
                  <Icon className="w-6 h-6 md:w-8 md:h-8 text-purple-400 mx-auto mb-3 group-hover:text-purple-300 transition-colors" />
                  <h4 className="text-white font-semibold text-sm md:text-base mb-1">{title}</h4>
          ...
          <div className="space-y-6">
            {/* Current Status */}
            <div className="project-card wakanda-pattern">
              <h3 className="text-lg md:text-xl font-bold text-white mb-6 text-center">Current Status</h3>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2 md:gap-3">
                  {quickStats.map(({ label, value, icon: Icon, color }) => (
                    <div key={label} className="text-center p-3 border border-purple-500/20 rounded-lg hover:border-purple-400/40 transition-all hover:bg-purple-500/5 group">
                      <Icon className={`w-4 h-4 mx-auto mb-2 ${color} group-hover:text-purple-300 transition-colors`} />
                      <div className="text-purple-400 font-semibold text-xs md:text-sm">{value}</div>
            ...
            <div className="project-card">
              <h3 className="text-lg font-bold text-white mb-6 text-center">What I'm Looking For</h3>
              <div className="space-y-3">
                <div className="flex items-start gap-3 p-3 md:p-4 rounded-lg bg-gradient-to-r from-purple-500/10 to-purple-600/10 border border-purple-500/20 hover:border-purple-400/30 transition-all">
                  ...
                  <div>
                    <h4 className="text-white text-sm font-semibold mb-1">Software & Full-Stack Roles</h4>
                    <p className="text-gray-400 text-xs">React, Node.js, Spring Boot</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3 p-3 md:p-4 rounded-lg bg-gradient-to-r from-purple-600/10 to-purple-700/10 border border-purple-500/20 hover:border-purple-400/30 transition-all">
                  <div className="w-2 h-2 bg-purple-500 rounded-full mt-2 flex-shrink-0 animate-pulse" style={{ animationDelay: '0.5s' }}></div>
                  <div>
                    <h4 className="text-white text-sm font-semibold mb-1">AI & SaaS Projects</h4>
                    <p className="text-gray-400 text-xs">ML integration, Automation</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3 p-3 md:p-4 rounded-lg bg-gradient-to-r from-purple-700/10 to-purple-800/10 border border-purple-500/20 hover:border-purple-400/30 transition-all">
                  <div className="w-2 h-2 bg-purple-600 rounded-full mt-2 flex-shrink-0 animate-pulse" style={{ animationDelay: '1s' }}></div>
                  <div>
                    <h4 className="text-white text-sm font-semibold mb-1">Collaborative Teams</h4>
                    <p className="text-gray-400 text-xs">Agile, Growth, Impact</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="project-card text-center bg-gradient-to-br from-purple-900/20 to-black border-purple-500/30">
              <div className="relative">
                ...
                <h4 className="text-lg md:text-xl font-bold text-white mb-3">Ready to Innovate?</h4>
                ...
                <div className="space-y-3">
                  ...
                  <Button 
                    ...
                    onClick={() => window.open('https://drive.google.com/uc?export=download&id=1pETEU29LNqvUJ4qRHW20IonKuuIGeoOp', '_blank')}
                  >
